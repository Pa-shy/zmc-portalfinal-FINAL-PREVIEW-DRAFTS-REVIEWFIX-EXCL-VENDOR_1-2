...
async function submitPaynow() {
  ...
  try {
    const response = await fetch('<?php echo e(route("accreditation.renewals.payment.paynow", $renewal)); ?>', {
      method: 'POST',
      headers: {
    ...
    if (data.ok) {
      alert(data.message);
      window.location.href = '<?php echo e(route("accreditation.renewals.show", $renewal)); ?>';
    } else {
...
async function submitProof() {
  ...
  try {
    const response = await fetch('<?php echo e(route("accreditation.renewals.payment.proof", $renewal)); ?>', {
      method: 'POST',
      headers: {
        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
      },
...
<?php echo $__env->make('layouts.portal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/runner/workspace/resources/views/portal/renewals/payment.blade.php ENDPATH**/ ?>