...
<script>
  ...
  async function submitApplication() {
    ...
    const isCorrection = <?php echo json_encode(isset($draft) && !$draft->is_draft && ($draft->status ?? null) === \App\Models\Application::CORRECTION_REQUESTED, 15, 512) ?>;
    ...
    try {
      ...
      const submitUrl = isCorrection
        ? <?php echo json_encode(isset($draft) && !$draft->is_draft ? route('mediahouse.applications.resubmit', $draft ?? 0) : route('mediahouse.submit'), 512) ?>
        : <?php echo json_encode(route('mediahouse.submit'), 15, 512) ?>;
