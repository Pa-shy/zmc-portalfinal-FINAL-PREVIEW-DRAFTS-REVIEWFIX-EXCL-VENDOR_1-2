<?php $__env->startSection('title', 'Staff Performance Metrics'); ?>
                    ...
                    <table class="table table-sm table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-end"><?php echo e(number_format($staff->payments_verified)); ?></td>
                                <td class="text-end">
                                    <span class="badge bg-<?php echo e($staff->avg_turnaround_hours < 24 ? 'success' : ($staff->avg_turnaround_hours < 48 ? 'warning' : 'danger')); ?>">
                                        <?php echo e(number_format($staff->avg_turnaround_hours, 1)); ?>h
                                    </span>
                                </td>
                                <td class="text-end">
                                    <?php if($staff->avg_turnaround_hours < 24): ?>
                                        <i class="ri-checkbox-circle-fill text-success"></i>
                                    <?php elseif($staff->avg_turnaround_hours < 48): ?>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="fw-bold"><?php echo e($staff->name); ?></td>
                                <td><?php echo e(ucwords(str_replace('_', ' ', $staff->role))); ?></td>
                                <td class="text-end">
                                    <span class="badge bg-<?php echo e($staff->reassignment_count < 3 ? 'success' : ($staff->reassignment_count < 5 ? 'warning' : 'danger')); ?>">
                                        <?php echo e($staff->reassignment_count); ?>

                                    </span>
                                </td>
                                <td class="text-end">
                                    <?php if($staff->reassignment_count < 3): ?>
                                        <span class="badge bg-success">Low</span>
                                    <?php elseif($staff->reassignment_count < 5): ?>
    ...
    function initCharts() {
        ...
        if (officerProcessedCanvas) {
            new Chart(officerProcessedCanvas, {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($applicationsProcessed->pluck('name')->toArray()); ?>,
                    datasets: [{
                        label: 'Applications Processed',
                        data: <?php echo json_encode($applicationsProcessed->pluck('processed_count')->toArray()); ?>,
        ...
        if (registrarReviewCanvas) {
            new Chart(registrarReviewCanvas, {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($averageReviewTime->pluck('name')->toArray()); ?>,
                    datasets: [{
                        label: 'Average Review Time (Hours)',
                        data: <?php echo json_encode($averageReviewTime->pluck('avg_review_hours')->toArray()); ?>,
        ...
        if (approvalDistributionCanvas) {
            new Chart(approvalDistributionCanvas, {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($approvalDistribution->pluck('name')->toArray()); ?>,
                    datasets: [
                        {
                            label: 'Approved',
                            data: <?php echo json_encode($approvalDistribution->pluck('approved_count')->toArray()); ?>,
                            backgroundColor: '#10b981'
                        },
                        {
                            label: 'Rejected',
                            data: <?php echo json_encode($approvalDistribution->pluck('rejected_count')->toArray()); ?>,
