<?php $__env->startSection('title', 'Geographic Distribution'); ?>
            ...
            <div class="zmc-card">
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-hover mb-0">
                        ...
                        <tbody>
                            <?php $__currentLoopData = $accreditationsByRegion; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $region): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                            ...
                            ?>
                            <tr>
                                <td class="fw-bold"><?php echo e(ucwords($region->region ?? 'Unknown')); ?></td>
                                <td class="text-end"><?php echo e(number_format($region->count)); ?></td>
                                <td class="text-end">$<?php echo e(number_format($revenue->total ?? 0, 2)); ?></td>
                                <td class="text-end">
                                    <span class="badge bg-<?php echo e($avgProcessingDays < 5 ? 'success' : ($avgProcessingDays < 10 ? 'warning' : 'danger')); ?>">
                                        <?php echo e(number_format($avgProcessingDays, 1)); ?>

                                    </span>
                                </td>
                                <td class="text-end"><?php echo e(number_format($mediaHouses->count ?? 0)); ?></td>
                                <td class="text-end">
                                    <?php if($avgProcessingDays < 5): ?>
                                        <i class="ri-checkbox-circle-fill text-success"></i>
                                    <?php elseif($avgProcessingDays < 10): ?>
                    ...
                    <div class="col-md-3">
                        <div class="p-3 border rounded-3 text-center bg-light">
                            ...
                            <div class="h5 fw-bold mb-0 text-primary">
                                <?php echo e(ucwords($accreditationsByRegion->sortByDesc('count')->first()->region ?? 'N/A')); ?>

                            </div>
                            <div class="text-muted smaller"><?php echo e(number_format($accreditationsByRegion->sortByDesc('count')->first()->count ?? 0)); ?> accreditations</div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="p-3 border rounded-3 text-center bg-light">
                            ...
                            <div class="h5 fw-bold mb-0 text-success">
                                <?php echo e(ucwords($revenueByRegion->sortByDesc('total')->first()->region ?? 'N/A')); ?>

                            </div>
                            <div class="text-muted smaller">$<?php echo e(number_format($revenueByRegion->sortByDesc('total')->first()->total ?? 0, 2)); ?></div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="p-3 border rounded-3 text-center bg-light">
                            ...
                            <div class="h5 fw-bold mb-0 text-info">
                                <?php echo e(ucwords($processingTimeByRegion->sortBy('avg_processing_days')->first()->region ?? 'N/A')); ?>

                            </div>
                            <div class="text-muted smaller"><?php echo e(number_format($processingTimeByRegion->sortBy('avg_processing_days')->first()->avg_processing_days ?? 0, 1)); ?> days</div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="p-3 border rounded-3 text-center bg-light">
                            ...
                            <div class="h5 fw-bold mb-0 text-warning">
                                <?php echo e(ucwords($mediaHousesByRegion->sortByDesc('count')->first()->region ?? 'N/A')); ?>

                            </div>
                            <div class="text-muted smaller"><?php echo e(number_format($mediaHousesByRegion->sortByDesc('count')->first()->count ?? 0)); ?> houses</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    function initCharts() {
        ...
        if (accreditationsCanvas) {
            new Chart(accreditationsCanvas, {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($accreditationsByRegion->pluck('region')->map(fn($r) => ucwords($r))->toArray()); ?>,
                    datasets: [{
                        label: 'Accreditations',
                        data: <?php echo json_encode($accreditationsByRegion->pluck('count')->toArray()); ?>,
        ...
        if (revenueCanvas) {
            new Chart(revenueCanvas, {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($revenueByRegion->pluck('region')->map(fn($r) => ucwords($r))->toArray()); ?>,
                    datasets: [{
                        label: 'Revenue',
                        data: <?php echo json_encode($revenueByRegion->pluck('total')->toArray()); ?>,
        ...
        if (processingTimeCanvas) {
            new Chart(processingTimeCanvas, {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($processingTimeByRegion->pluck('region')->map(fn($r) => ucwords($r))->toArray()); ?>,
                    datasets: [{
                        label: 'Avg. Processing Days',
                        data: <?php echo json_encode($processingTimeByRegion->pluck('avg_processing_days')->toArray()); ?>,
        ...
        if (mediaHousesCanvas) {
            new Chart(mediaHousesCanvas, {
                type: 'pie',
                data: {
                    labels: <?php echo json_encode($mediaHousesByRegion->pluck('region')->map(fn($r) => ucwords($r))->toArray()); ?>,
                    datasets: [{
                        data: <?php echo json_encode($mediaHousesByRegion->pluck('count')->toArray()); ?>,
...
<?php echo $__env->make('layouts.portal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/runner/workspace/resources/views/staff/director/reports/geographic.blade.php ENDPATH**/ ?>