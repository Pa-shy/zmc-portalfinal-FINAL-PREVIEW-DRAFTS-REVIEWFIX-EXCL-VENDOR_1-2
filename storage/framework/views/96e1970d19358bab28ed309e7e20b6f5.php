<?php $__env->startSection('title', 'Accreditation Officer Dashboard'); ?>
...
<div class="zmc-dashboard-wrapper" style="font-family:'Roboto', sans-serif; color:#334155;">
      ...
      <div class="zmc-card h-100">
        <div class="d-flex justify-content-between align-items-start">
          <div>
            ...
            <div class="h3 fw-black mb-0"><?php echo e($summaryCorrections); ?></div>
          </div>
      ...
      <div class="zmc-card h-100">
        <div class="d-flex justify-content-between align-items-start">
          <div>
            ...
            <div class="h3 fw-black mb-0"><?php echo e($summaryRejected); ?></div>
          </div>
        ...
        <div class="row g-2 mb-3">
          <div class="col-6">
            <div class="border rounded p-2 text-center">
              <div class="h4 fw-bold mb-0"><?php echo e($totalAccredited); ?></div>
              <div class="small text-muted">Total</div>
            </div>
          </div>
          <div class="col-6">
            <div class="border rounded p-2 text-center">
              <div class="h4 fw-bold mb-0 text-warning"><?php echo e($uncollectedCards); ?></div>
              <div class="small text-muted">Uncollected</div>
            </div>
          </div>
        </div>
        <a href="<?php echo e(route('staff.officer.records.accredited-journalists')); ?>" class="btn btn-outline-primary w-100">
    ...
    <div class="col-12 col-md-6">
      <div class="zmc-card h-100">
        ...
        <div class="row g-2 mb-3">
          <div class="col-6">
            <div class="border rounded p-2 text-center">
              <div class="h4 fw-bold mb-0"><?php echo e($totalRegistered); ?></div>
              <div class="small text-muted">Total</div>
            </div>
          </div>
          <div class="col-6">
            <div class="border rounded p-2 text-center">
              <div class="h4 fw-bold mb-0 text-warning"><?php echo e($uncollectedCerts); ?></div>
              <div class="small text-muted">Uncollected</div>
            </div>
          </div>
        </div>
        <a href="<?php echo e(route('staff.officer.records.registered-mediahouses')); ?>" class="btn btn-outline-success w-100">
  ...
  <?php if(($expiringJournalists && $expiringJournalists->count() > 0) || ($expiringMediaHouses && $expiringMediaHouses->count() > 0)): ?>
    <div class="zmc-card mb-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="<?php echo e(route('staff.officer.renewals.expiring')); ?>" class="btn btn-sm btn-outline-primary">
          <i class="ri-eye-line me-1"></i> View All
        </a>
      </div>

      <div class="row g-3">
        <?php if($expiringJournalists && $expiringJournalists->count() > 0): ?>
          <div class="col-12 col-lg-6">
            <div class="border rounded p-3">
              <div class="d-flex justify-content-between align-items-center mb-2">
                <div class="fw-bold small text-muted">Media Practitioners (<?php echo e($expiringJournalists->count()); ?>)</div>
                <form method="POST" action="<?php echo e(route('staff.officer.renewals.send-reminders')); ?>" class="d-inline">
                  <?php echo csrf_field(); ?>
              ...
              <div class="table-responsive">
                <table class="table table-sm table-hover align-middle mb-0">
                  ...
                  <tbody>
                    <?php $__currentLoopData = $expiringJournalists; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rec): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                      <?php
                      ...
                      ?>
                      <tr>
                        <td class="small"><?php echo e($rec->holder->name ?? '—'); ?></td>
                        <td class="small"><?php echo e($rec->certificate_no ?? '—'); ?></td>
                        <td class="small"><?php echo e(optional($rec->expires_at)->format('d M Y') ?? '—'); ?></td>
                        <td class="text-end">
        ...
        <?php if($expiringMediaHouses && $expiringMediaHouses->count() > 0): ?>
          <div class="col-12 col-lg-6">
            <div class="border rounded p-3">
              <div class="d-flex justify-content-between align-items-center mb-2">
                <div class="fw-bold small text-muted">Media Houses (<?php echo e($expiringMediaHouses->count()); ?>)</div>
                <form method="POST" action="<?php echo e(route('staff.officer.renewals.send-reminders')); ?>" class="d-inline">
                  <?php echo csrf_field(); ?>
              ...
              <div class="table-responsive">
                <table class="table table-sm table-hover align-middle mb-0">
                  ...
                  <tbody>
                    <?php $__currentLoopData = $expiringMediaHouses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rec): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                      <?php
                        $days = $rec->expires_at ? now()->startOfDay()->diffInDays($rec->expires_at->startOfDay(), false) : null;
                        $urgency = $days !== null && $days <= 30 ? 'danger' : ($days <= 60 ? 'warning' : 'info');
                      ?>
                      <tr>
                        <td class="small"><?php echo e($rec->entity_name ?? '—'); ?></td>
                        <td class="small"><?php echo e($rec->registration_no ?? '—'); ?></td>
                        <td class="small"><?php echo e(optional($rec->expires_at)->format('d M Y') ?? '—'); ?></td>
                        <td class="text-end">
                          <span class="badge bg-<?php echo e($urgency); ?>-subtle text-<?php echo e($urgency); ?> border border-<?php echo e($urgency); ?>-subtle">
                            <?php echo e($days ?? '—'); ?> days
  ...
  <?php if($inactiveJournalists && $inactiveJournalists->count() > 0): ?>
    <div class="zmc-card mb-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="fw-bold m-0">
          <i class="ri-user-unfollow-line me-2" style="color:#dc3545"></i> Inactive Media Practitioners (2-3 Years)
        </h6>
        <span class="badge bg-danger"><?php echo e($inactiveJournalists->count()); ?> inactive</span>
      </div>
      ...
      <div class="table-responsive">
        <table class="table table-sm table-hover align-middle mb-0">
          ...
          <tbody>
            <?php $__currentLoopData = $inactiveJournalists; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rec): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <?php
              ...
              <tr>
                <td class="small"><?php echo e($rec->holder->name ?? '—'); ?></td>
                <td class="small"><?php echo e($rec->certificate_no ?? '—'); ?></td>
                <td class="small"><?php echo e($lastActivity ? $lastActivity->format('d M Y') : 'Never'); ?></td>
                <td class="text-end">
                  <span class="badge bg-danger-subtle text-danger border border-danger-subtle">
                    <?php echo e($daysInactive ?? '—'); ?> days
                  </span>
                </td>
              </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <?php $__empty_1 = true; $__currentLoopData = $applications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $app): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          ...
          <tr>
            ...
            <td>
              ...
              ?>
              <span class="badge bg-<?php echo e($reqBadge); ?>"><?php echo e(ucfirst($reqType)); ?></span>
            </td>
            ...
            <td>
              <span class="badge rounded-pill bg-<?php echo e($badge); ?> px-3"><?php echo e(ucwords(str_replace('_',' ', $status ?: '—'))); ?></span>
            </td>
            ...
            <td class="text-end">
              <div class="zmc-action-strip">
                ...
                <button
                  ...
                  data-target="#corrModal<?php echo e($app->id); ?>"
                  <?php if(!$canDecide): ?> disabled <?php endif; ?>
                ...
                <button
                  ...
                  data-target="#approveModal<?php echo e($app->id); ?>"
                  <?php if(!$canDecide): ?> disabled <?php endif; ?>
                  data-bs-toggle="tooltip" data-bs-placement="top"
                  title="Approve"
                >
          ...
          <?php $__env->startPush('zmc_modals'); ?>
            ...
            <div class="modal fade zmc-chat-modal" id="corrModal<?php echo e($app->id); ?>" tabindex="-1" aria-hidden="true">
              <div class="modal-dialog modal-dialog-centered modal-lg">
                <form class="modal-content" method="POST" action="<?php echo e($correctionUrl($app->id)); ?>">
                  <?php echo csrf_field(); ?>
                  ...
                  <div class="modal-header zmc-modal-header">
                    <div class="d-flex align-items-center gap-2">
                      ...
                      <div>
                        <div class="zmc-modal-title">Request correction <span class="ms-2 text-muted" style="font-weight:800;font-size:12px;"><?php echo e($ref); ?></span></div>
                        <div class="zmc-modal-sub">Applicant: <span class="fw-bold"><?php echo e($app->applicant?->name ?? '—'); ?></span></div>
                      </div>
                    </div>
            ...
            <div class="modal fade" id="approveModal<?php echo e($app->id); ?>" tabindex="-1" aria-hidden="true">
              <div class="modal-dialog modal-dialog-centered">
                <form class="modal-content" method="POST" action="<?php echo e($approveUrl($app->id)); ?>">
                  ...
                  <div class="modal-header zmc-modal-header">
                    <div>
                      <div class="zmc-modal-title"><i class="fa-solid fa-check me-2" style="color:var(--zmc-accent-dark)"></i>Approve application <span class="ms-2 text-muted" style="font-weight:800;font-size:12px;"><?php echo e($ref); ?></span></div>
...
<?php echo $__env->make('layouts.portal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/runner/workspace/resources/views/staff/officer/dashboard.blade.php ENDPATH**/ ?>