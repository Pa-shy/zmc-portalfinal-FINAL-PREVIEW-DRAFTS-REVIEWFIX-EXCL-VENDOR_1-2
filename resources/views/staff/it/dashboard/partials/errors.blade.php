            ...
            <table class="table table-hover align-middle m-0">
                ...
                <tbody>
                    ...
                    <tr class="table-danger-light">
                        <td class="ps-4 small fw-medium text-slate-600">{{ $log->created_at->format('Y-m-d H:i:s') }}</td>
                        <td class="small fw-bold text-danger">{{ strtoupper($log->action) }}</td>
                        <td>
                            <div class="small fw-bold text-slate-900">{{ $log->user?->email ?? 'Guest' }}</div>
                            <div class="text-slate-600 fw-bold" style="font-size: 11px;">IP: {{ $log->ip }}</div>
                        </td>
                        <td style="max-width: 400px;">
                            <code class="small text-danger bg-danger-subtle p-1 rounded">{{ json_encode($log->meta) }}</code>
                        </td>
                    </tr>
                    @empty
                    <tr>
