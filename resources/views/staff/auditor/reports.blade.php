...
<div class="zmc-dashboard-wrapper" style="font-family:'Roboto', sans-serif; color:#334155;">
  ...
  <div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-4">
    ...
    <a class="btn btn-white border btn-sm" href="{{ route('staff.auditor.dashboard') }}"><i class="ri-arrow-left-line me-1"></i>Back</a>
  </div>
  ...
  <div class="zmc-card shadow-sm border-0 p-3 mb-4">
    <form method="GET" class="row g-2 align-items-end">
      ...
      <div class="col-md-6 d-flex gap-2">
        ...
        <a class="btn btn-white border btn-sm" href="{{ route('staff.auditor.reports') }}">Reset</a>
        <a class="btn btn-sm btn-success" href="{{ route('staff.auditor.reports.csv', request()->query()) }}"><i class="ri-download-2-line me-1"></i>Export CSV</a>
        ...
        <div class="card-body p-4">
          <div class="d-flex justify-content-between align-items-start">
            <div class="flex-grow-1">
              ...
              <div class="display-4 fw-black mb-2">{{ number_format($total) }}</div>
              @if($from && $to)
        ...
        <div class="card-body p-4">
          <div class="d-flex justify-content-between align-items-start">
            <div class="flex-grow-1">
              ...
              <div class="display-4 fw-black mb-2">{{ number_format($approved) }}</div>
              <div class="small" style="opacity: 0.85;">
  ...
  <div class="row g-3 mb-4">
      ...
      <div class="zmc-card shadow-sm border-0 p-4 h-100">
        <div class="d-flex justify-content-between align-items-start mb-3">
          <div class="flex-grow-1">
            ...
            <div class="fw-black" style="font-size:32px; color:#3b82f6;">{{ number_format($stats['paynow_confirmed'] ?? 0) }}</div>
          </div>
        ...
        <div class="progress" style="height: 10px; border-radius: 5px;">
          <div class="progress-bar bg-primary" style="width: {{ $paynowRate }}%"></div>
        </div>
        <div class="d-flex justify-content-between mt-2">
          <small class="text-muted">{{ round($paynowRate, 1) }}% of total</small>
          @if($total > 0)
            <small class="text-muted">{{ $total - ($stats['paynow_confirmed'] ?? 0) }} pending</small>
          @endif
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="zmc-card shadow-sm border-0 p-4 h-100">
        ...
        @endphp
        <div class="progress" style="height: 10px; border-radius: 5px;">
          <div class="progress-bar bg-success" style="width: {{ $proofsRate }}%"></div>
        </div>
        <div class="d-flex justify-content-between mt-2">
          <small class="text-muted">{{ round($proofsRate, 1) }}% of total</small>
          @if($total > 0)
            <small class="text-muted">{{ $total - ($stats['proofs_approved'] ?? 0) }} pending</small>
          @endif
        </div>
      </div>
    </div>
      ...
      <div class="zmc-card shadow-sm border-0 p-4 h-100">
        <div class="d-flex justify-content-between align-items-start mb-3">
          <div class="flex-grow-1">
            ...
            <div class="fw-black" style="font-size:32px; color:#f59e0b;">{{ number_format($stats['waivers_approved'] ?? 0) }}</div>
          </div>
        ...
        @endphp
        <div class="progress" style="height: 10px; border-radius: 5px;">
          <div class="progress-bar bg-warning" style="width: {{ $waiversRate }}%"></div>
        </div>
        <div class="d-flex justify-content-between mt-2">
          <small class="text-muted">{{ round($waiversRate, 1) }}% of total</small>
          @if($total > 0)
            <small class="text-muted">{{ $total - ($stats['waivers_approved'] ?? 0) }} standard</small>
          @endif
        </div>
      </div>
    </div>
  ...
  @if(($stats['paynow_confirmed'] ?? 0) > 0 || ($stats['proofs_approved'] ?? 0) > 0 || ($stats['waivers_approved'] ?? 0) > 0)
  <div class="row g-3 mb-4">
    <div class="col-md-12">
      <div class="zmc-card shadow-sm border-0 p-4">
      ...
      <table class="table table-borderless align-middle">
        ...
        <tbody>
          <tr class="border-top">
            ...
            <td class="py-3 text-end fw-bold" style="font-size: 16px;">{{ number_format($total) }}</td>
          ...
          <tr class="border-top">
            ...
            <td class="py-3 text-end" style="font-size: 15px;">{{ number_format($approved) }}</td>
          ...
          <tr>
            ...
            <td class="py-3 text-end" style="font-size: 15px;">{{ number_format($total - $approved - $rejected) }}</td>
            <td class="py-3 text-end pe-3">{{ round(100 - $approvalRate - $rejectionRate, 1) }}%</td>
          </tr>
          <tr class="border-top bg-light">
            <td class="py-3 ps-3"><i class="ri-bank-card-line text-primary me-2"></i>PayNow Confirmed</td>
            <td class="py-3 text-end" style="font-size: 15px;">{{ number_format($stats['paynow_confirmed'] ?? 0) }}</td>
            <td class="py-3 text-end pe-3">{{ round($paynowRate, 1) }}%</td>
          </tr>
          <tr class="bg-light">
            <td class="py-3 ps-3"><i class="ri-file-check-line text-success me-2"></i>Proofs Approved</td>
            <td class="py-3 text-end" style="font-size: 15px;">{{ number_format($stats['proofs_approved'] ?? 0) }}</td>
            <td class="py-3 text-end pe-3">{{ round($proofsRate, 1) }}%</td>
          </tr>
          <tr class="bg-light">
            <td class="py-3 ps-3"><i class="ri-hand-coin-line text-warning me-2"></i>Waivers Approved</td>
            <td class="py-3 text-end" style="font-size: 15px;">{{ number_format($stats['waivers_approved'] ?? 0) }}</td>
            <td class="py-3 text-end pe-3">{{ round($waiversRate, 1) }}%</td>
          </tr>
        </tbody>
      </table>
    </div>
