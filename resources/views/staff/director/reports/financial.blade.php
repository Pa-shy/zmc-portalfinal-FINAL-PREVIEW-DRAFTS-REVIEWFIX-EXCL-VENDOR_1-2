    ...
    <div class="row g-4">
        ...
        <div class="col-md-4">
            <div class="zmc-card h-100 border-start border-4 border-warning">
                ...
                <div class="display-6 fw-black text-warning">{{ $waivers->count }}</div>
                <div class="text-muted">Waivers granted</div>
                <hr>
                <div class="h4 fw-bold text-dark">${{ number_format($waivers->waived_amount, 2) }}</div>
            ...
            <div class="zmc-card h-100">
                ...
                <div class="row text-center mt-4">
                    <div class="col-4 border-end">
                        <div class="h2 fw-black text-success">{{ $aging['0_30'] }}</div>
                        <div class="text-muted smaller">0-30 Days</div>
                    </div>
                    <div class="col-4 border-end">
                        <div class="h2 fw-black text-warning">{{ $aging['30_60'] }}</div>
                        <div class="text-muted smaller">30-60 Days</div>
                    </div>
                    <div class="col-4">
                        <div class="h2 fw-black text-danger">{{ $aging['60_plus'] }}</div>
    ...
    if (typeof Chart !== 'undefined') {
        ...
        if (serviceCanvas) {
            new Chart(serviceCanvas, {
                type: 'bar',
                data: {
                    labels: {!! json_encode($revenueByService->pluck('service_type')->toArray()) !!},
                    datasets: [{
                        label: 'Revenue ($)',
                        data: {!! json_encode($revenueByService->pluck('total_revenue')->toArray()) !!},
        ...
        if (typeCanvas) {
            new Chart(typeCanvas, {
                type: 'pie',
                data: {
                    labels: {!! json_encode($revenueByType->pluck('applicant_category')->toArray()) !!},
                    datasets: [{
                        data: {!! json_encode($revenueByType->pluck('total_revenue')->toArray()) !!},
